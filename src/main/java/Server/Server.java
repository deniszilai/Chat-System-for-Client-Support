 ...
 * and load/save messages history on start/shut down
 */
public class Server
{
    ...
    // Current connected user usernames.
     ...
     * of a client. In the case of a connection, it's RPC, and it returns true
     * (or false if not correctly done) with the messages, and connected users lists
     */
    private void onConnection(String consumerTag, Delivery delivery) throws IOException
    {
        // Get the data.
        Data.Connection connection = SerializationUtils
    ...
    private void createHomeDir()
    {
        ...
        try
        {
            ...
            {
                System.err.println("Error: cannot create the chat app home directory " +
                        HOME_DIR_PATH + ".");
                System.exit(-1);
            }
        }
